...
```html
<script src="https://cdn.jsdelivr.net/npm/inko@1.1.2/inko.min.js"></script>
```
...
The package includes type declarations for CommonJS and ES module consumers.

```ts
import Inko, { type InkoOption } from 'inko';

const options: InkoOption = { allowDoubleConsonant: true };
const inko = new Inko(options);
```

For CommonJS TypeScript projects without `esModuleInterop`, use `import Inko = require('inko')`.

## Callbacks

Conversion methods keep their instance configuration when used as callbacks:

```js
const { ko2en } = new Inko();
['안녕', 'ㄻㄳ'].map(ko2en); // ['dkssud', 'fart']
```

## Supported keyboard layout

Inko converts the standard Korean two-set (두벌식) keyboard layout. Three-set (세벌식) support is tracked in [#11](https://github.com/738/inko/issues/11).

Shifted consonants retain their original key mapping: `ㅉㅈ.` becomes `Ww.`. Inko does not guess whether the intended text was `www.`; that enhancement is tracked in [#23](https://github.com/738/inko/issues/23).

## Development

Use Node.js 22 or newer and npm to work on the library:

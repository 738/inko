...
describe('inko', () => {
  ...
  it('영타 -> 한글 (복자음 허용 X)', () => {
    ...
    assert.equal(
      inko.en2ko('rsefaqtdwczxvg', { allowDoubleConsonant: false }),
      'ㄱㄴㄷㄹㅁㅂㅅㅇㅈㅊㅋㅌㅍㅎ'
    );
  });
  ...
  it('영타 -> 한글 (복자음 허용 O)', () => {
    ...
    assert.equal(
      inko.en2ko('rldjr wjvusdp sjrk todrkrsk', o()),
      '기억 저편에 너가 생각나'
    );
    assert.equal(inko.en2ko('dmlrlthcla', o()), '의기소침');
    assert.equal(
      inko.en2ko('dho wjgksxp rmfjtpdy?', o()),
      '왜 저한테 그러세요?'
    );
    assert.equal(
      inko.en2ko('woalTwlaks woaldjqtek.', o()),
      '재밌지만 재미없다.'
    );
    assert.equal(
      inko.en2ko('dbfrhrdldltjstodsladms djswpsk skgksxp clswjfgktuTek.', o()),
      '율곡이이선생님은 언제나 나한테 친절하셨다.'
    );
    assert.equal(
      inko.en2ko('alclwldksgrhtjdi rmfjf tn djqtdmf rjtdlek.', o()),
      '미치지않고서야 그럴 수 없을 것이다.'
    );
    ...
    assert.equal(
      inko.en2ko('r s e f a q t d w c z x v g', o()),
      'ㄱ ㄴ ㄷ ㄹ ㅁ ㅂ ㅅ ㅇ ㅈ ㅊ ㅋ ㅌ ㅍ ㅎ'
    );
    assert.equal(inko.en2ko('hlhl', o()), 'ㅚㅚ');
    assert.equal(inko.en2ko('QlEkrgkrp', o()), '삐딱하게');
    assert.equal(
      inko.en2ko('ekfkawnl gjs cptqkznldp xkrhvk', o()),
      '다람쥐 헌 쳇바퀴에 타고파'
    );
    assert.equal(
      inko.en2ko('EKFKAWNL GJS CPTQKZNLDP XKRHVK', o()),
  ...
  describe('설정 부여 관련', () => {
    it('constructor에 allowDoubleConsonant: true 설정 부여', () => {
      const instance = new Inko({ allowDoubleConsonant: true });
      assert.equal(instance.en2ko('rtrt'), 'ㄳㄳ');
      assert.equal(
        instance.en2ko('rsefaqtdwczxvg'),
        'ㄱㄴㄷㄻㅄㅇㅈㅊㅋㅌㅍㅎ'
      );
    });

    it('config로 설정을 켜고 끄거나 기본값으로 초기화', () => {
      const instance = new Inko({ allowDoubleConsonant: false });
      assert.equal(instance.en2ko('rtrt'), 'ㄱㅅㄱㅅ');
      instance.config({ allowDoubleConsonant: true });
      assert.equal(instance.en2ko('rtrt'), 'ㄳㄳ');
      instance.config({ allowDoubleConsonant: false });
      assert.equal(instance.en2ko('rtrt'), 'ㄱㅅㄱㅅ');
      instance.config({ allowDoubleConsonant: true });
      instance.config();
      assert.equal(instance.en2ko('rtrt'), 'ㄱㅅㄱㅅ');
    ...
    it('함수에 전달한 설정은 해당 호출에만 적용', () => {
      const instance = new Inko();
      assert.equal(
        instance.en2ko('rtrt', { allowDoubleConsonant: true }),
        'ㄳㄳ'
      );
      assert.equal(
        instance.en2ko('rsefaqtdwczxvg', { allowDoubleConsonant: true }),
        'ㄱㄴㄷㄻㅄㅇㅈㅊㅋㅌㅍㅎ'
      );
      assert.equal(instance.en2ko('rtrt'), 'ㄱㅅㄱㅅ');
    });
